# src/my_package_name/views/image_viewer.py
import os
from typing import List
from PyQt5.QtWidgets import (
    QGraphicsView, QMainWindow, QShortcut,
...
class ImageViewerWindow(QMainWindow):
    ...
    def __init__(self, files: List[str], index: int = 0) -> None:
        ...
        for key,mode in [('D','pan'),('R','rect'),('P','poly'),('E','select')]:
            ...
            mode_grp.addAction(act); tb.addAction(act)
    ...
    def _update_status(self, *args) -> None:
        ...
        if len(args)==2:
            x,y = args; msg = f"Mode: {m} | Zoom: {z}% | Pos: ({x},{y})"
        else:
            msg = f"Mode: {m} | Zoom: {z}%"
    ...
    def _save_annotations(self) -> None:
        self.ann_mgr.save(self.files[self.index],
                          self.viewer.scene_obj.annotations)
